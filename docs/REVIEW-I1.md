...
| 13 | **Medium — test trust (I0)** | The API suite failed about one test in four runs, in a different place each time: a 401 from a valid session, a 404 from a route that exists, sometimes `Parse Error: Expected HTTP/`. Handed a server that is not listening, supertest binds a fresh ephemeral port for **every** request — some 1,500 per run — and occasionally a request reached a listener that was going away. It reads exactly like a race in the application and is nothing of the kind. | The test app listens once per file (`app.listen(0, '127.0.0.1')`) and every request goes to that one server. Seven consecutive clean runs, where three in four had failed. `TEST_TRACE=1` prints the app and database lifecycle with the test that caused it — that is what ruled out the application and found the harness. |
| 14 | Low — correctness (I0) | The system settings were cached in-process for five seconds. A setting here is a *rule* (`locked_through`, `allow_negative_stock`): with a second API replica, one keeps applying the old rule — the defect the I0 review found in the permission cache. | Read per request; it is a primary-key scan of a dozen rows. |
